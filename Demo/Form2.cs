    ...
    public partial class Form2 : Form
    {
        ...
        private void Form2_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (moved)
            {
                ...
                if (CheckGameOver())
                {
                    SaveHighScore(); // Lưu điểm cao trước khi hiển thị thông báo
                    DialogResult dia = MessageBox.Show("Điểm của bạn là: " + score.ToString() + "\n" + "Bạn có muốn chơi lại không?",
        ...
        private void ResetGame()
        {
            ...
            score = 0;
            initCards();
            lbHighScore.Text = highScore.ToString(); // Cập nhật lại điểm cao
        }

        // Tải điểm cao từ file
        private void LoadHighScore()
        {
            if (File.Exists("highscore_5x5.txt"))
            {
                highScore = int.Parse(File.ReadAllText("highscore_5x5.txt"));
            }
        }
        ...
        {
            File.WriteAllText("highscore_5x5.txt", highScore.ToString());
        }
        ...
        private void ExitToMenu_Click(object sender, EventArgs e)
        {
            if (!menuFormOpened) // Kiểm tra nếu MenuForm chưa mở
            {
                ...
                menu.Show();
                this.Close(); // Chỉ cần Close form hiện tại, không cần Hide
            }
        }
    }
}
